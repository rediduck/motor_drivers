...
#    define MOTOR_DEFAULT_MODE_DM                                                                  \
        MOTOR_CTRL_INTERNAL_VEL // 考虑到需要对舵轮实现速度位置双重控制，遂只用DM内部速度环，外部设置位置环；如果需要使用达妙位置环需要自己define
...
#define __MOTOR_CTRL_DISABLE(__CTRL_HANDLE__) ((__CTRL_HANDLE__)->enable = false)
     ...
     * @param threshold 阈值
     */
    static inline void Motor_PosCtrl_SetErrorThreshold(Motor_PosCtrl_t* hctrl,
