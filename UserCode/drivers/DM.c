...
void DM_Init(DM_t* hdm, const DM_Config_t dm_config)
{
    ...
    hdm->inv_reduction_rate = 1.0f /                                                            // 取倒数将除法转为乘法加快运算速度
                              ((dm_config.reduction_rate > 0 ? dm_config.reduction_rate : 1.0f) // 外接减速比
...
{
    float scale_angle = 2.0f * hdm->POS_MAX_RAD / 65535.0f; // 读取到的浮点数是和16位位置数据成线性关系，计算k值
    float scale_vel   = 2.0f * hdm->VEL_MAX_RAD / 4095.0f;  // 读取到的浮点数是和12位位置数据成线性关系，计算k值
    float scale_t     = 2.0f * hdm->T_MAX / 4095.0f;

    const float feedback_angle = scale_angle * (uint16_t)(data[1] << 8 | data[2]) - hdm->POS_MAX_RAD;    // 反馈位置数据，达妙3519和2520反馈的数据是减速前的
    const float feedback_vel   = scale_vel * (uint16_t)(data[3] << 4 | data[4] >> 4) - hdm->VEL_MAX_RAD; // 反馈速度数据，达妙3519和2520反馈的数据是减速后的
    ...
    hdm->abs_angle = (hdm->reverse ? -1.0f : 1.0f) * // 反转时需要反转角度输入
                     ((float)hdm->round_cnt * 360.0f + angle - hdm->angle_zero) * hdm->inv_reduction_rate;
...
static void dm_pos_set_command_data(DM_t* hdm, const float value_vel_rad, const float value_angle_rad, uint8_t data[])
{
    uint8_t* vbuf;
    uint8_t* pbuf;
...
void DM_Vel_SendSetCmd(DM_t* hdm, const float value_vel)
{
    ...
    float value_vel_rad = value_vel * 2 * 3.1416f / 60.0f; // 达妙电机控制的即为输出轴的速度（uint:rad/s）
