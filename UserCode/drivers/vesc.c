 ...
 * @param data 数据缓冲区
 */
static void get_set_command_data(VESC_t*                    hvesc,
                                 const VESC_CAN_PocketSet_t pocket_id,
                                 const float                value,
                                 uint8_t                    data[])
{
    int32_t data_value;
...
static void get_conf_command_data(const VESC_CAN_PocketConf_t pocket_id,
                                  const float                 value0,
                                  const float                 value1,
                                  uint8_t                     data[8])
{
 ...
 * @param data 数据
 */
void VESC_CAN_DataDecode(VESC_t*                       hvesc,
                         ...
                         const uint8_t                 data[8])
{
    ...
    case VESC_CAN_STATUS_4:
        ...
        hvesc->abs_angle    = (float) hvesc->feedback.round_cnt * 360.0f + hvesc->feedback.pos -
                           hvesc->angle_zero;
        break;
 ...
 * VESC_CAN_Fifo0ReceiveCallback); 来注册回调函数
 * @param hcan
 */
